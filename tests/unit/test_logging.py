...
class TestLoggingUtils:
    ...
    def test_update_logging_handlers(self: TestLoggingUtils) -> None:
        ...
        python_logger = logging.getLogger("testing")
        tango_logger = mock.MagicMock(spec=tango.Logger)

        def null_creator(target: str, _: tango.Logger) -> logging.NullHandler:
        ...
        try:
            ...
            LoggingUtils.update_logging_handlers(
                new_targets, python_logger, tango_logger
            )
            assert len(python_logger.handlers) == 1
            mocked_creator.assert_called_once_with("console::cout", tango_logger)
            # test same handler is retained for same request
            old_handler = python_logger.handlers[0]
            new_targets = ["console::cout"]
            ...
            assert len(python_logger.handlers) == 1
            assert python_logger.handlers[0] is old_handler
            ...
            assert len(python_logger.handlers) == 4
            assert mocked_creator.call_count == 3
            mocked_creator.assert_has_calls(
                [
                    mock.call("file::/tmp/dummy", tango_logger),
                    mock.call("syslog::some/address", tango_logger),
                    mock.call("tango::logger", tango_logger),
                ],
                any_order=True,
            )
            ...
            assert len(python_logger.handlers) == 2
            mocked_creator.assert_not_called()
            ...
            assert len(python_logger.handlers) == 0
            mocked_creator.assert_not_called()
